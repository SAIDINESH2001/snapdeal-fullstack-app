...
export const PaymentModal = ({ show, handleClose }) => {
  ...
  const { cartProducts, quantities, subTotal, savedAddresses, loading, refreshCart } = useCart(show);
  const { user } = useAuth();
          ...
          <Col md={7} className="p-4 bg-white border-end" style={{ minHeight: '500px' }}>
                      ...
                      savedAddresses.map((addr) => (
                        ...
                        >
                          ...
                          {tempSelectedAddressId === addr._id && (
                            <span className="material-symbols-outlined text-danger position-absolute top-0 end-0 p-2 fs-6">check_circle</span>
                          )}
                        </div>
                      ))
                    ) : (
                      <div className="text-center p-4 border rounded text-muted small bg-light">No address found. Click add new.</div>
                    )}
                  </div>
                </div>
              </>
            ...
            {step === 'address' && (
              <div className="address-form">
                ...
                <Row className="g-3">
                  ...
                  <Col md={6}><Form.Control size="sm" placeholder="Country" value={newAddress.country} disabled /></Col>
                </Row>
                <div className="mt-4 d-flex gap-2">
                  <Button variant="danger" size="sm" className="px-4 fw-bold" style={{ borderRadius: '3px' }} onClick={handleAddressSubmit} disabled={isProcessing}>SAVE ADDRESS</Button>
                  <Button variant="light" size="sm" className="px-4 fw-bold border" style={{ borderRadius: '3px' }} onClick={() => setStep('summary')}>CANCEL</Button>
                </div>
              </div>
            )}
            ...
            {step !== 'address' && (
              <div className="mt-auto pt-4 border-top d-flex align-items-center justify-content-between">
                ...
                <Button
                  disabled={isProcessing || (step === 'summary' && !activeAddress)}
                  style={{ background: '#e40046', border: 'none', padding: '10px 40px', borderRadius: '3px', fontWeight: 'bold' }}
            ...
            <div className="pe-2" style={{ maxHeight: '250px', overflowY: 'auto' }}>
              {cartProducts.map((item) => (
                <div key={item._id} className="d-flex gap-3 mb-3 pb-3 border-bottom align-items-start">
                  ...
                  <div className="fw-bold small">₹{(item.sellingPrice * (quantities?.[item._id] || 1)).toLocaleString()}</div>
                </div>
              ))}
            </div>
