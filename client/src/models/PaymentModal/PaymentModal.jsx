...
export const PaymentModal = ({
  ...
  const { cartProducts, quantities, savedAddresses, loading, refreshCart } =
    useCart(show);
  const { user } = useAuth();
  ...
  const activeAddress =
    savedAddresses?.find((addr) =>
      tempSelectedAddressId
        ? addr._id === tempSelectedAddressId
        : addr.isDefault,
    ) || savedAddresses?.[0];
  ...
  const handlePayment = async () => {
    ...
    const shippingData = {
      ...
      phone: user?.phone || activeAddress.phone,
    };
  ...
  return (
    ...
    >
      ...
      >
        <span className="fw-bold fs-5 fs-md-4 ms-2" style={{ letterSpacing: "-1px" }}>
          snapdeal
        </span>
        <span
      ...
      <div className="bg-white px-3 px-md-4 py-2 border-bottom shadow-sm">
        <div
        ...
        >
          {step !== "summary" && (
            <span
              className="material-symbols-outlined fs-6 fs-md-5 me-2 cursor-pointer text-muted d-none d-md-inline"
          ...
          <span
            className={
              `${step === "summary" ? "fw-bold text-danger" : "text-muted"} text-nowrap`}
          >
            Summary
          </span>
          ...
          <span
            className={
             `${step === "address" ? "fw-bold text-danger" : "text-muted"} text-nowrap`} 
          >
            Add Address
          </span>
          <span className="text-muted opacity-50 mx-1">»</span>
          <span
            className={
              `${step === "payment" ? "fw-bold text-danger" : "text-muted"} text-nowrap`}
          >
            Payments
          </span>
        </div>
      ...
      <Modal.Body className="p-0">
        <Row className="g-0 flex-column-reverse flex-md-row">
          <Col
            xs={12}
            md={7}
            className="p-3 p-md-4 bg-white border-md-end"
          ...
          >
            {step === "summary" && (
              <>
                <div className="mb-4 border-bottom pb-3">
                  ...
                  <div className="ps-3 text-muted small">
                    {user?.phone ? `+91 ${user.phone}` : "+91 9987456255"}
                  </div>
                </div>
                <div className="mb-4">
                  ...
                  >
                    {savedAddresses?.map((addr) => (
                      <div
                        key={addr._id}
                        className={`p-3 rounded mb-2 border cursor-pointer position-relative ${tempSelectedAddressId === addr._id ? "border-danger bg-light" : ""}`}
                      ...
                      >
                        ...
                        {tempSelectedAddressId === addr._id && (
                          <span className="material-symbols-outlined text-danger position-absolute top-0 end-0 p-2 fs-6">
                            check_circle
                          </span>
                        )}
                      </div>
              ...
              <div className="payment-selection">
                ...
                <div className="border rounded overflow-hidden">
                  {[
                    ...
                    { id: "cod", label: "COD", icon: "local_shipping" },
                  ].map((method) => (
                    <div
                      key={method.id}
                    ...
                    >
                      <span className="material-symbols-outlined text-danger me-3">
                        {method.icon}
                      </span>
                      <span className="flex-grow-1 fw-bold small">
                        {method.label}
                      </span>
            ...
            {step !== "address" && (
              <div className="mt-auto pt-4 border-top d-flex flex-column flex-sm-row align-items-center justify-content-between gap-3">
                <div className="fw-bold fs-5">
                  ₹ {activeSubTotal.toLocaleString()}
                </div>
                <Button
                  disabled={
                    isProcessing || (step === "summary" && !activeAddress)
                  }
                  ...
                  onClick={() =>
                    step === "summary" ? setStep("payment") : handlePayment()
                  }
                >
                  {isProcessing
                    ? "Processing..."
          ...
          <Col xs={12} md={5} className="p-3 p-md-4 border-top border-md-top-0" style={{ backgroundColor: "#fcfcfc" }}>
            ...
            >
              {activeProducts.map((item) => (
                <div
                  key={item._id}
                  className="d-flex gap-3 mb-3 pb-3 border-bottom align-items-start"
                >
                  <img
                    src={item.image?.[0]}
                    width="40"
                  ...
                  <div className="flex-grow-1" style={{ fontSize: "11px" }}>
                    ...
                    <div className="text-muted">
                      Qty: {activeQuantities[item._id] || 1}
                    </div>
                  ...
                  <div className="fw-bold small">
                    ₹
                    {(
                      item.sellingPrice * (activeQuantities[item._id] || 1)
                    ).toLocaleString()}
                  </div>
                </div>
              ))}
            ...
            <div className="mt-4 border-top pt-2">
              <div className="d-flex justify-content-between fw-bold fs-6">
                ...
                <span>₹ {activeSubTotal.toLocaleString()}</span>
              </div>
            </div>
          </Col>
        </Row>
