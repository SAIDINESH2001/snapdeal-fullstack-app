...
export const ProductSideBar = ({ products = [], onSortChange }) => {
  ...
  const sidebarHierarchy = useMemo(() => {
    if (subCategory === "all") {
      return { parent: mainCategory, active: category };
    }
    if (category === "all") {
      return { parent: null, active: mainCategory };
    }
    return { parent: mainCategory, active: subCategory || category };
  ...
  const { availableColors, availableBrands, availableDiscounts, filteredProducts } = useMemo(() => {
    ...
    return {
      ...
      availableColors: Object.keys(colorMap).map(n => ({ 
        name: n, 
        count: colorMap[n], 
        hex: n.toLowerCase() === "multicolor" ? "linear-gradient(to right, red, orange, yellow, green, blue, indigo, violet)" : n.toLowerCase() 
      })),
    ...
    <div className="container-fluid mt-3 bg-white">
      <div className="row">
        <aside className="col-md-3 col-lg-2 border-end px-3 sidebar-filters">
          <CategoryDisplay 
            mainCategory={sidebarHierarchy.parent} 
            category={sidebarHierarchy.active} 
          ...
          <FilterSection title="Brand" isOpen={openSections.brand} onToggle={() => toggleSection('brand')}>
            <BrandFilter 
              ...
              availableBrands={availableBrands.filter(b => b.name.toLowerCase().includes(brandSearch.toLowerCase()))} 
