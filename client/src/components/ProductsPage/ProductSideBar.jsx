...
export const ProductSideBar = ({ products = [], onSortChange }) => {
  ...
  const colorPalette = [
    "Red", "Blue", "Black", "White", "Green", "Yellow", "Grey", "Pink", "Purple", 
    "Brown", "Orange", "Beige", "Navy", "Maroon", "Gold", "Silver", "Multicolor", "Cream", "Teal"
  ];
  ...
  const { availableColors, availableBrands, availableDiscounts, filteredProducts } = useMemo(() => {
    ...
    const brandMap = {};
    const discountMap = {};
    discountRanges.forEach(range => { discountMap[range.label] = 0; });

    const productsWithDetectedColor = products.map(p => {
      const searchText = `${p.name} ${p.description || ""}`.toLowerCase();
      let foundColor = p.color;
      if (!foundColor) {
        foundColor = colorPalette.find(color => searchText.includes(color.toLowerCase()));
      }
    ...
    return {
      ...
      availableColors: Object.keys(colorMap).map(name => ({
        name,
        count: colorMap[name],
        hex: name.toLowerCase() === "multicolor" ? "linear-gradient(to right, red, orange, yellow, green, blue, indigo, violet)" : name.toLowerCase()
      })),
      availableBrands: Object.keys(brandMap).map(name => ({ name, count: brandMap[name] })),
      availableDiscounts: discountRanges.map(range => ({ ...range, count: discountMap[range.label] })).filter(r => r.count > 0)
    };
    ...
    <div className="container-fluid mt-3 bg-white">
      <div className="row">
        <aside className="col-md-3 col-lg-2 border-end px-3 sidebar-filters">
          <div className="mb-4 pb-2 border-bottom">
            ...
            <ul className="list-unstyled mb-0" style={{ fontSize: "13px" }}>
              ...
              <li className="ps-3 mb-2 d-flex align-items-center justify-content-between">
                ...
                <span className="text-muted" style={{ fontSize: "11px" }}>{filteredProducts.length}</span>
              </li>
            </ul>
          </div>

          <div className="mb-4 pb-2 border-bottom">
            <div className="d-flex justify-content-between align-items-center mb-2" style={{ cursor: "pointer" }} onClick={() => setIsBrandOpen(!isBrandOpen)}>
              <h6 className="fw-bold mb-0" style={{ fontSize: "14px" }}>Brand</h6>
              <span className="fw-bold text-muted">{isBrandOpen ? "−" : "+"}</span>
            </div>
            {isBrandOpen && (
              <>
                <div className="input-group input-group-sm mb-2 border-bottom">
                  <input 
                    ...
                    style={{ fontSize: "12px" }}
                  />
                  <span className="input-group-text bg-white border-0"><span className="material-symbols-outlined text-muted" style={{fontSize: "16px"}}>search</span></span>
                </div>
                    ...
                    .map((brand) => (
                      <div key={brand.name} className="form-check small d-flex align-items-center mb-1">
                        <input 
                          ...
                          onChange={() => handleToggle(brand.name, selectedBrands, setSelectedBrands)} 
                        />
                        <label className="form-check-label text-muted flex-grow-1" style={{cursor: 'pointer'}} onClick={() => handleToggle(brand.name, selectedBrands, setSelectedBrands)}>{brand.name}</label>
          ...
          <div className="mb-4 pb-2 border-bottom">
            <div className="d-flex justify-content-between align-items-center mb-2" style={{ cursor: "pointer" }} onClick={() => setIsColorOpen(!isColorOpen)}>
              <h6 className="fw-bold mb-0" style={{ fontSize: "14px" }}>Color</h6>
              <span className="fw-bold text-muted">{isColorOpen ? "−" : "+"}</span>
            </div>
            {isColorOpen && (
              <div className="filter-scroll" style={{ maxHeight: "200px", overflowY: "auto" }}>
                {availableColors.length > 0 ? availableColors.map((color) => (
                  <div key={color.name} className="form-check small d-flex align-items-center mb-1">
                    <input className="form-check-input rounded-0 me-2 border shadow-none" type="checkbox" checked={selectedColors.includes(color.name)} onChange={() => handleToggle(color.name, selectedColors, setSelectedColors)} />
                    <span className="me-2 border" style={{ width: "14px", height: "14px", background: color.hex, display: "inline-block" }}></span>
                    <label className="form-check-label text-muted flex-grow-1" style={{cursor: 'pointer'}} onClick={() => handleToggle(color.name, selectedColors, setSelectedColors)}>{color.name}</label>
                    <span className="text-muted small ms-2">{color.count}</span>
                  </div>
                )) : <div className="small text-muted ps-2">No colors detected</div>}
              </div>
            )}
          </div>

          <div className="mb-4 pb-2 border-bottom">
            <div className="d-flex justify-content-between align-items-center mb-2" style={{ cursor: "pointer" }} onClick={() => setIsDiscountOpen(!isDiscountOpen)}>
              <h6 className="fw-bold mb-0" style={{ fontSize: "14px" }}>Discount %</h6>
              <span className="fw-bold text-muted">{isDiscountOpen ? "−" : "+"}</span>
            </div>
            {isDiscountOpen && (
                ...
                {availableDiscounts.map((range) => (
                  <div key={range.label} className="form-check small d-flex align-items-center mb-1">
                    <input 
                      ...
                      onChange={() => setSelectedDiscount(selectedDiscount?.label === range.label ? null : range)} 
                    />
                    <label className="form-check-label text-muted flex-grow-1" style={{cursor: 'pointer'}} onClick={() => setSelectedDiscount(selectedDiscount?.label === range.label ? null : range)}>{range.label}</label>
